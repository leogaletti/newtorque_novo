...
if($_POST){
	...
	if($HTTP_POST_FILES['file1']['tmp_name']){
		$id_query = mysql_query("INSERT INTO fotos_servicos_executados(cd_servico_executado,ds_legenda)
								 VALUES('{$cod_servico_executado}','{$_POST[ds_legenda1]}')")
								  or die(mysql_error());
		$id_query = mysql_query("SELECT * FROM fotos_servicos_executados ORDER BY cd_foto_servico_executado DESC");
		...
		copy("../imagens/temp.jpg","../fotos_servicos_executados/foto_{$arr[cd_foto_servico_executado]}.JPG");
		copy("../imagens/temp_mini.jpg","../fotos_servicos_executados/mini_foto_{$arr[cd_foto_servico_executado]}.JPG");
	}
	if($HTTP_POST_FILES['file2']['tmp_name']){
		$id_query = mysql_query("INSERT INTO fotos_servicos_executados(cd_servico_executado,ds_legenda)
								 VALUES('{$cod_servico_executado}','{$_POST[ds_legenda2]}')")
								  or die(mysql_error());
		$id_query = mysql_query("SELECT * FROM fotos_servicos_executados ORDER BY cd_foto_servico_executado DESC");
		$arr = mysql_fetch_assoc($id_query);
		$quality = 85;
	...
	if($HTTP_POST_FILES['file3']['tmp_name']){
		$id_query = mysql_query("INSERT INTO fotos_servicos_executados(cd_servico_executado,ds_legenda)
								 VALUES('{$cod_servico_executado}','{$_POST[ds_legenda3]}')")
								  or die(mysql_error());
		$id_query = mysql_query("SELECT * FROM fotos_servicos_executados ORDER BY cd_foto_servico_executado DESC");
		$arr = mysql_fetch_assoc($id_query);
		$quality = 85;
	...
	if($HTTP_POST_FILES['file4']['tmp_name']){
		$id_query = mysql_query("INSERT INTO fotos_servicos_executados(cd_servico_executado,ds_legenda)
								 VALUES('{$cod_servico_executado}','{$_POST[ds_legenda4]}')")
								  or die(mysql_error());
		$id_query = mysql_query("SELECT * FROM fotos_servicos_executados ORDER BY cd_foto_servico_executado DESC");
		$arr = mysql_fetch_assoc($id_query);
		$quality = 85;
	...
	if($HTTP_POST_FILES['file5']['tmp_name']){
		$id_query = mysql_query("INSERT INTO fotos_servicos_executados(cd_servico_executado,ds_legenda)
								 VALUES('{$cod_servico_executado}','{$_POST[ds_legenda5]}')")
								  or die(mysql_error());
		$id_query = mysql_query("SELECT * FROM fotos_servicos_executados ORDER BY cd_foto_servico_executado DESC");
		$arr = mysql_fetch_assoc($id_query);
		$quality = 85;
...
<table width="100%" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	<form action="cadastro_fotos_servico_executado.php" method="post" enctype="multipart/form-data">
	<tr>
	...
	<tr>
	  <td colspan="2" align="center" class="style2">Novas FOTOS para o  SERVI&Ccedil;O - &quot;<? echo($nome_servico_executado) ?>&quot;</td>
	  </tr>
	<tr>
