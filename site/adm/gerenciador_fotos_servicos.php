...
<table width="100%" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	<form action="gerenciador_fotos_servicos.php" method="post">
	<tr> 
	...
	<table width="100%" border="1" align="center" cellpadding="0" cellspacing="4">
      <tr align="left">
        <td colspan="9" align="center" class="style2">Gerenciar de FOTOS do SERVI&Ccedil;O - &quot;<? echo($nome_servico) ?>&quot;          <?
if(isset($_POST[id])){
	$id = $_POST[id];
	if($_POST[acao] == "Remover"){
       	foreach($id as $i){
			$id_query = mysql_query("DELETE FROM fotos_servicos WHERE cd_foto_servico = '$i'");
			unlink("../fotos_servicos/mini_foto_"."$i.JPG");
			unlink("../fotos_servicos/foto_"."$i.JPG");
		}
	}elseif($_POST[acao] == "Publicar"){
       	foreach($id as $i){
			$id_query = mysql_query("UPDATE fotos_servicos SET bl_publicarfs = '1' WHERE cd_foto_servico = '$i'");
		}
	}elseif($_POST[acao] == "Despublicar"){
       	foreach($id as $i){
			$id_query = mysql_query("UPDATE fotos_servicos SET bl_publicarfs = '0' WHERE cd_foto_servico = '$i'");
		}
...
if(isset($_POST[ord])){
	...
	if($_POST[acao] == "Salvar Ordem"){
		foreach($ord as $o => $cd_ordem){
			$id_query = mysql_query("UPDATE fotos_servicos SET cd_ordemfs='$cd_ordem' WHERE cd_foto_servico = '$o'");
		}
	}elseif(isset($_POST[acao])){
...
$id_query = mysql_query("Select * from fotos_servicos inner join servicos on fotos_servicos.cd_servico = servicos.cd_servico WHERE fotos_servicos.cd_servico = $cod_servico ORDER BY $novaordem") or die("Erro ao executar consulta");
if (!mysql_num_rows($id_query)) {
	echo("<p><font color=\"red\"><b>N&atilde;o h&aacute; nenhuma foto inclu&iacute;da neste &aacute;lbum. <a href=\"cadastro_fotos_servico.php?cd_serv=$cod_servico&nm_serv=$nome_servico\">clique aqui para incluir uma nova</a></b></font></p>");
}else{
?></td>
      </tr>
      <tr>
        ...
        <td align="center"><a href="cadastro_fotos_servico.php?cd_serv=<? echo($cod_servico) ?>&amp;nm_serv=<? echo($nome_servico) ?>"><img src="tit_adm_incluir.gif" width="59" height="21" border="0" /></a></td>
      </tr>
      <?
	while($arr = mysql_fetch_assoc($id_query)){
?>
      	<tr bgcolor="#CCCCCC">
		<td align="center"><input type="checkbox" value="<? echo($arr[cd_foto_servico]) ?>" name="id[]" style="border-width: 0px"></td>
        <td align="center">
<?
			$ImageSize  = GetImageSize("../fotos_servicos/foto_$arr[cd_foto_servico].JPG");
			$Img_w = $ImageSize[0]; 
			$Img_h = $ImageSize[1]; 
?>
        <a href="javascript:foto('../fotos_servicos/foto_<? echo($arr[cd_foto_servico]) ?>.JPG','<? echo($Img_w) ?>','<? echo($Img_h) ?>')"> <img src="../fotos_servicos/mini_foto_<? echo($arr[cd_foto_servico]) ?>.JPG" alt="Clique para ver a foto em tamanho real" border="0"></a>
        </td>
		<td align="center"><input type="text" size="2" value="<?=$arr[cd_ordemfs]?>" name="ord[<?=$arr[cd_foto_servico]?>]" /></td>
...
?>
        ...
        <td align="center"><a href="edicao_fotos_servico.php?cd=<? echo($arr[cd_foto_servico]) ?>&amp;cd_serv=<? echo($arr[cd_servico]) ?>&amp;nm_serv=<? echo($arr[tit_servico]) ?>"><img src="tit_adm_editar.gif" width="70" height="23" border="0"></a></td>
      </tr>
      <?
	}
?>
