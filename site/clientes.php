...
<link rel="stylesheet" href="js/coin-slider-styles.css" type="text/css" media="screen" />
...
<div class="newt-content-layout layout-item-1">
    ...
    <div class="newt-layout-cell layout-item-2" style="width: 75%; background-attachment: scroll;	 background-image: url(imagens/fundo_conteudo.gif); background-repeat: no-repeat;  background-position: top; height:600px;" >
        <p> <span style="color: rgb(201, 29, 32); font-size: 24px; font-weight: bold; line-height: 24px; -webkit-border-horizontal-spacing: 2px; -webkit-border-vertical-spacing: 2px;">Clientes</span></p>
        
<p>
<?
	$id_query = mysql_query("SELECT * FROM paginas");
	$arr1 = mysql_fetch_assoc($id_query);
	echo($arr1[ds_pag_clientes]);
?>
</p>
<br />
        
        <table class="newt-article newt-preview-show-borders-table" border="0" cellspacing="0" cellpadding="0" style="margin-top: 4px; margin-bottom: 4px; margin-left: 4px; margin-right: 4px; width: 100%;">
            <tbody>
                <tr>
<?
	$id_query = mysql_query("SELECT * FROM clientes where bl_publicar= '1' order by cd_ordem");
	$conta_fotos = 1;
...
?>
        <td class="newt-preview-show-borders-cell" style="text-align: center; padding-top: 4px; padding-right: 4px; padding-bottom: 4px; padding-left: 4px; width: 33%;"><a href="<? echo($arr[ds_link]) ?>" target="<? echo($arr[ds_target]) ?>"><img src="fotos_cliente/foto_<? echo($arr[cd_cliente])?>.JPG" title="<? echo($arr[ds_link]) ?>" hspace="4" vspace="2" border="1"></a><br /><br /><br /></td>
