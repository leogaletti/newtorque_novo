...
<META NAME="Keywords" Content="new torque servicos_executados prestacao equipamentos torquimetro valvulas medidores venda">
...
<!-- O arquivo lightbox.css faz parte da biblioteca e é necessária sua inclusão-->
...
<div class="newt-content-layout layout-item-1">
    ...
    <div class="newt-layout-cell layout-item-2" style="width: 75%; background-attachment: scroll;	 background-image: url(imagens/fundo_conteudo.gif); background-repeat: no-repeat;  background-position: top; height:600px;" >
        <p><span style="color: rgb(201, 29, 32); font-size: 24px; font-weight: bold; line-height: 24px; -webkit-border-horizontal-spacing: 2px; -webkit-border-vertical-spacing: 2px;">Servi&ccedil;os Executados</span></p>
        
        <p><span style="color: rgb(201, 29, 32); font-size: 24px; font-weight: bold; line-height: 24px; -webkit-border-horizontal-spacing: 2px; -webkit-border-vertical-spacing: 2px;"></span>        
<?
	$id_query_serv = mysql_query("Select * from servicos_executados where cd_servico_executado = '{$_GET[cd]}' and bl_publicars ='1'");
	$arr_serv = mysql_fetch_assoc($id_query_serv);
?>
        <h4><? echo($arr_serv[tit_servico_executado])?></h4>
</p>
    <div class="newt-layout-cell layout-item-2" style="width: 50%;">
        <table class="newt-article" border="0" cellspacing="0" cellpadding="0" style="width: 100%;">
            <tbody>
                ...
                <tr>
                    <td class="" style="padding-top: 8px; padding-right: 8px; padding-bottom: 8px; padding-left: 8px;"><h3><? echo($arr_serv[ds_servico_executado]) ?></h3></td>
                </tr>
            </tbody>
        </table>
    </div>
...
<?
				$id_query = mysql_query("Select * from fotos_servicos_executados where cd_servico_executado = '{$_GET[cd]}' and bl_publicarfs ='1' ORDER BY cd_ordemfs");
...
?>
                        <a href="fotos_servicos_executados/foto_<? echo($arr[cd_foto_servico_executado]) ?>.JPG" rel="lightbox[roadtrip]" title="<? echo($arr[ds_legenda]) ?>"><img src="fotos_servicos_executados/mini_foto_<? echo($arr[cd_foto_servico_executado]) ?>.JPG" title="<? echo($arr[ds_legenda]) ?>" border="1" class="thumb"></a>
<?
						$conta_fotos += 1;
					}
				}
