...
<div class="newt-content-layout layout-item-1">
    ...
    <div class="newt-layout-cell layout-item-2" style="width: 75%; background-attachment: scroll;	 background-image: url(imagens/fundo_conteudo.gif); background-repeat: no-repeat; background-position:top; height:600px;" >
        ...
        <p><span style="color: rgb(201, 29, 32); font-size: 24px; font-weight: bold; line-height: 24px; -webkit-border-horizontal-spacing: 2px; -webkit-border-vertical-spacing: 2px;"></span>Tecnologia de ponta, essa &eacute; a melhor express&atilde;o para os equipamentos que a New Torque comercializa. Dispon&iacute;veis para venda e loca&ccedil;&atilde;o, a empresa oferece um completo portf&oacute;lio de m&aacute;quinas e acess&oacute;rios para os trabalhos de torque de precis&atilde;o.<br />
        <br /></p>
        
<p>
<ul>
<?
	$id_query_linha = mysql_query("SELECT * FROM linhas where bl_publicar='1' ORDER BY cd_linha");
	while($arr_linha = mysql_fetch_assoc($id_query_linha)){
?>
        <li>
		<h4><? echo($arr_linha[tit_linha]) ?></h4>
		<ul>
<?
		$id_query = mysql_query("Select * from equipamentos where cd_linha = '{$arr_linha[cd_linha]}' and bl_publicare ='1' ORDER BY cd_equipamento ASC");
