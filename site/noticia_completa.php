...
<style type="text/css">
.newt-post .layout-item-0 { border-top-style:solid;border-right-style:solid;border-bottom-style:solid;border-left-style:solid;border-top-width:1px;border-right-width:1px;border-bottom-width:1px;border-left-width:1px;border-top-color:#3F97A6;border-right-color:#3F97A6;border-bottom-color:#3F97A6;border-left-color:#3F97A6; padding-right: 5px;padding-left: 5px; }
...
<div class="newt-content-layout">
    <div class="newt-content-layout-row">
    <div class="newt-layout-cell layout-item-0" style="width: 75%; background-attachment: scroll; background-image: url(imagens/fundo_conteudo.gif); background-position:top; background-repeat: no-repeat; height:600px;">
        <p><span style="color: rgb(201, 29, 32); font-size: 24px; font-weight: bold; line-height: 24px; -webkit-border-horizontal-spacing: 2px; -webkit-border-vertical-spacing: 2px;">Not&iacute;cias</span></p>
<p>
<?
    $id_query = mysql_query("SELECT * FROM noticias where cd_noticia = '{$_GET[cd]}'") or die("Erro ao executar consulta");
    $arr = mysql_fetch_assoc($id_query);
?>
	<b><? echo($arr[tit_noticia]) ?></b>
</p>
<?
	if(empty($arr[ds_fonte])){
	}else{
?>
		<p>Fonte: <? echo($arr[ds_fonte]) ?></p>
<?
	}
?>
<p>
...
<p>
	<? echo("Outras Notícias") ?>
        <ul>
<?
	$sql = "SELECT * FROM noticias where bl_publicar='1' ORDER BY cd_ordem";
	$id_query = mysql_query($sql);
	while($arr = mysql_fetch_assoc($id_query)){
?>
            <li>                  
        	<a href="noticia_completa.php?cd=<? echo($arr[cd_noticia]) ?>"><? echo($arr[tit_noticia]) ?></a>
            </li>
<?
	}
?>
    ...
    </div><div class="newt-layout-cell layout-item-0" style="width: 200px;">
