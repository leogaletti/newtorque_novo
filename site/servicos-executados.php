...
<?
	...
	echo($arr1[ds_pag_servexec]);
?>
</p>
<p>
<ul>
<?
	$id_query = mysql_query("SELECT * FROM servicos_executados where bl_publicars= '1' ORDER BY cd_ordems");
	if (mysql_num_rows($id_query)){
		while($arr = mysql_fetch_assoc($id_query)){
?>
                    <li>
                    <a href="fotos-servicos-executados.php?cd=<? echo($arr[cd_servico_executado]) ?>"><? echo($arr[tit_servico_executado]) ?><br /></a>
                    </li>
					<br />
<?
		}
